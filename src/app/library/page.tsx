...
import { deleteAssetById, listImageAssets, revokeObjectURL, getAssetObjectURL } from '@/lib/assets';
...
function LibraryPageContent() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            const urls = await Promise.all(records.map(r => getAssetObjectURL(r.id)));
            if (!mounted) return;
            const mapped: LibraryItem[] = records.map((r, i) => ({
                id: r.id,
                name: r.name,
                mimeType: r.mimeType,
            ...
            <div className="flex-1 flex flex-col min-h-0 w-full h-full">
                ...
                <div className="flex-1 min-h-0">
                    <div className="mx-auto max-w-6xl px-4 py-4">
                        {loading ? (
                            <div className="text-sm text-neutral-500 dark:text-neutral-400">Loading images…</div>
                        ) : items.length === 0 ? (
                            <div className="text-sm text-neutral-500 dark:text-neutral-400">No images yet. Attach images in chat to see them here.</div>
                        ) : (
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                                {items.map(item => (
                                    <figure key={item.id} className="group relative overflow-hidden rounded-md border border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900">
                                        <img
                                            src={item.url}
