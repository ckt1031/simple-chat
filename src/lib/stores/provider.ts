...
export interface ProviderStore {
    ...
    addCustomModel: (providerId: string, model: Pick<Model, 'id' | 'name'>) => boolean;
    updateModel: (providerId: string, modelId: string, updates: Partial<Pick<Model, 'id' | 'name' | 'enabled'>>) => boolean;
    removeCustomModel: (providerId: string, modelId: string) => void;
}

const createOfficialProvider = (provider: OfficialProvider): OfficialProviderState => ({
    type: 'official',
    provider,
    enabled: false,
    apiKey: '',
...
const createCustomProvider = (id: string, providerFormat: OfficialProvider): CustomProviderState => ({
    type: 'custom',
    id,
    providerFormat,
    enabled: false,
...
export const useProviderStore = create<ProviderStore>()(
    persist(
        (set, get) => ({
            providers: {
                [OfficialProvider.OPENAI]: createOfficialProvider(OfficialProvider.OPENAI),
                [OfficialProvider.GOOGLE]: createOfficialProvider(OfficialProvider.GOOGLE),
                [OfficialProvider.OPENROUTER]: createOfficialProvider(OfficialProvider.OPENROUTER),
            },
            ...
            hasEnabledProviders: () => {
                return get().getAllProviders().some(provider => provider.enabled);
            },
            ...
            addCustomModel: (providerId: string, model: Pick<Model, 'id' | 'name'>) => {
                let success = false;
                set((state) => {
                    const provider = state.providers[providerId];
                    if (!provider) return state;
            ...
            updateModel: (providerId: string, modelId: string, updates: Partial<Pick<Model, 'id' | 'name' | 'enabled'>>) => {
                let success = false;
                set((state) => {
                    const provider = state.providers[providerId];
                    if (!provider) return state;
                    ...
                    if (nextId !== modelId && provider.models.some((m) => m.id === nextId)) {
                        return state;
                    }

                    const updated: Model = { ...provider.models[index], ...updates, id: nextId } as Model;
                ...
                set((state) => {
                    ...
                    return {
                        providers: {
                            ...
                            [providerId]: {
                                ...provider,
                                models: provider.models.filter((m) => !(m.source === 'custom' && m.id === modelId)),
                            },
                        },
                    };
                });
