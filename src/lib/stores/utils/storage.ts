...
export const ConversationIndxedStorage: StateStorage = {
  getItem: async (name: string): Promise<string | null> => {
    const index: ConversationIndex | undefined = await get(
    ...
    for (const id of index.ids) {
      const conv = (await get(buildConversationKey(id), conversationStore)) as
        | StoredConversation
        | undefined;
  ...
  setItem: async (name: string, value: string): Promise<void> => {
    ...
    const conversations: StoredConversation[] = Array.isArray(
      state.conversations,
    )
      ? state.conversations
      : [];

    const previousIndex: ConversationIndex | undefined = await get(
      conversationIndexKey,
      conversationStore,
    );
  ...
  removeItem: async (name: string): Promise<void> => {
    ...
    if (index?.ids?.length) {
      await Promise.all(
        index.ids.map((id) => del(buildConversationKey(id), conversationStore)),
      );
    }
